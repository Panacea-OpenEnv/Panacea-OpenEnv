"""Panacea POMDP environment (server-side).

Each episode the agent sees a hospital resource claim. It can call up to 8 tools
to gather evidence (each costs a small reward), then submit APPROVED/REJECTED.
Some claims are deceptive (ghost patient, cost inflation, comorbidity masking,
collusion); the reward function rewards correct verdicts and bonuses agents
that backed their call with evidence from the right tool.
"""
...
class PanaceaEnvironment(Environment):
    ...
    def step(self, action: OversightAction, timeout_s=None, **kwargs) -> OversightObservation:
        ...
        if self._step_count > MAX_STEPS:
            self._done = True
            return self._build_observation(done=True, reward=-2.0, last_evidence="")

        # Older callers send only {verdict, reasoning} without the type field.
        ...
        if action_type == "tool_call":
            ...
            return self._build_observation(done=False, reward=cost, last_evidence=evidence)

        # Verdict path: terminal.
        ...
        obs = self._build_observation(done=True, reward=total_reward, last_evidence="")
        obs.metadata.update({
            "ground_truth":     gt,
            "deception_type":   deception_type,
    ...
    def _build_observation(self, done: bool, reward, last_evidence: str) -> OversightObservation:
        s = self._scenario
